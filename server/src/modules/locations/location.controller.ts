...
import { createLocationSchema, patchLocationSchema } from './location.validation.js';
...
export const createLocationHandler = async (
  ...
  try {
    ...
    const location = await createWorkLocation(req.tenantId, parsed.data, req.user.sub, {
      ip: req.ip,
