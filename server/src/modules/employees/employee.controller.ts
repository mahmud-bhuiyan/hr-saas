...
export const updateMyEmployeeHandler = async (
  req: AuthenticatedRequest,
  res: Response,
): Promise<void> => {
  try {
    const parsed = updateMyEmployeeSchema.safeParse(req.body);
    if (!parsed.success) {
      res.status(400).json({
        status: "error",
    ...
    const employee = await updateMyEmployee(
      req.tenantId!,
      req.user!.sub,
      parsed.data,
      auditContext(req),
...
export const createEmployeeLoginHandler = async (
  req: AuthenticatedRequest,
  res: Response,
): Promise<void> => {
  try {
    const parsed = createEmployeeLoginSchema.safeParse(req.body ?? {});
    if (!parsed.success) {
      res.status(400).json({
        status: "error",
        message: parsed.error.issues[0]?.message ?? "Invalid request body",
    ...
    const result = await createEmployeeLogin(
      req.tenantId!,
      req.params.id,
      parsed.data,
      req.user!.sub,
