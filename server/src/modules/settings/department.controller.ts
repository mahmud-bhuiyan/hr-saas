...
import { createDepartmentSchema, patchDepartmentSchema } from './department.validation.js';
...
export const createDepartmentHandler = async (
  ...
  try {
    ...
    const department = await createDepartment(req.tenantId, parsed.data, req.user.sub);
