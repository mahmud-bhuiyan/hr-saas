...
const toForm = (settings: {
  ...
  multiStepApprovalEnabled: settings.multiStepApprovalEnabled ? 'true' : 'false',
});
...
export const LeaveSettingsPage = () => {
  ...
  const saveMutation = useMutation({
    ...
    onError: (error: unknown) => {
      toast.error(error instanceof ApiError ? error.message : 'Failed to save leave policy');
    },
  });
      ...
      <div className="card-surface space-y-5 p-6">
        <FormField label="Annual entitlement (days)" htmlFor="annual-entitlement">
          <Input
            ...
            onChange={(event) =>
              setForm((prev) => ({ ...prev, annualEntitlement: event.target.value }))
            }
            icon={<HiCalendarDays className="h-4 w-4 text-brand-600" />}
          />
          <p className="mt-1.5 text-sm text-slate-500">
            Pro-rata applies automatically for mid-year starters based on employee start date.
          </p>
        </FormField>
        ...
        <FormField label="Max carry-over days" htmlFor="max-carry-over">
          <Input
            ...
            onChange={(event) =>
              setForm((prev) => ({ ...prev, maxCarryOverDays: event.target.value }))
            }
            icon={<HiCalendarDays className="h-4 w-4 text-brand-600" />}
          />
        </FormField>
        ...
        <FormField label="Multi-step approval">
          <Select
            ...
            onChange={(event) =>
              setForm((prev) => ({ ...prev, multiStepApprovalEnabled: event.target.value }))
            }
          ...
          <p className="mt-1.5 text-sm text-slate-500">
            When enabled, managers approve step 1 and HR gives final approval on step 2.
          </p>
        </FormField>
