...
export const AttendanceEmployeeCorrections = ({
  onCorrect,
}: AttendanceEmployeeCorrectionsProps) => {
  ...
  const employeesQuery = useQuery({
    ...
    queryFn: () =>
      fetchEmployees({ status: "active", search: search || undefined }),
  });
    ...
    <div className="space-y-6">
      ...
      {!employeeId ? (
        <p className="rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600 dark:border-slate-700 dark:bg-slate-800/50 dark:text-slate-300">
          Select an employee to view attendance history and correct missed
          punches.
        </p>
      ) : (
        <div>
          ...
          {(historyQuery.data?.total ?? 0) >
            ...
            <div className="mt-4 flex justify-center gap-2">
              ...
              <span className="px-2 py-1 text-sm text-slate-600 dark:text-slate-400">
                Page {page}
              </span>
              <button
                type="button"
                className="rounded-lg border border-slate-200 px-3 py-1 text-sm disabled:opacity-50 dark:border-slate-700"
                disabled={
                  page * (historyQuery.data?.limit ?? 20) >=
                  (historyQuery.data?.total ?? 0)
                }
                onClick={() => setPage((current) => current + 1)}
              >
