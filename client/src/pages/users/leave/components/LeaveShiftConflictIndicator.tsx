...
const shiftStatusLabel = (
  status: LeaveShiftConflictSummary["status"],
): string => {
  switch (status) {
    case "draft":
      return "Draft";
...
export const LeaveShiftConflictIndicator = ({
  conflicts,
}: LeaveShiftConflictIndicatorProps) => {
  if (!conflicts?.length) {
  ...
  return (
    <div className="max-w-md rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-left text-xs text-red-900 dark:border-red-500/30 dark:bg-red-500/10 dark:text-red-200">
      <p className="font-semibold">
        {conflicts.length} scheduled shift{conflicts.length === 1 ? "" : "s"}{" "}
        overlap
      </p>
