...
export const formatAttendanceTime = (iso: string, use24Hour: boolean): string => {
...
export const getCurrentWeekDates = (): string[] => {
  ...
  for (let i = 0; i < 7; i += 1) {
    const d = new Date(
      Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate() + diffToMonday + i)
    );
...
export type AttendanceLogsTab = 'attendance-log' | 'calendar' | 'attendance-requests' | 'overtime-requests';
...
export const buildMondayFirstCalendarDays = (year: number, month: number): KekaCalendarDay[] => {
...
export const getMonthStrip = (year: number, month: number, count = 7): Array<{ year: number; month: number; label: string }> => {
...
export const formatKekaMonthYear = (year: number, month: number): string => {
  const name = new Date(Date.UTC(year, month - 1, 1)).toLocaleString(undefined, { month: 'long' });
  return `${name}, ${year}`;
};
