...
export const AttendanceCalendar = ({
  ...
  return (
    <div className="keka-card overflow-hidden">
      ...
      >
        <h3 className="text-base font-semibold">{formatKekaMonthYear(year, month)}</h3>
      ...
      {loading ? (
        <div className="flex h-72 items-center justify-center keka-muted text-sm">Loading calendar…</div>
      ) : (
        <div className="flex flex-col lg:flex-row">
          <div className="min-w-0 flex-1">
            <div
              className="grid grid-cols-7 border-b text-center text-[10px] font-semibold uppercase tracking-wide"
              style={{ borderColor: 'var(--keka-border)', color: 'var(--keka-muted)' }}
            >
              {WEEK_HEADERS.map((label) => (
                <div key={label} className="border-r py-2 last:border-r-0" style={{ borderColor: 'var(--keka-border)' }}>
                  {label}
                </div>
              ))}
            </div>
              ...
              {calendarCells.map((cell) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => cell.inCurrentMonth && onDaySelect(cell.date)}
                  ...
                  >
                    ...
                    {dayData && dayData.totalMinutes > 0 && cell.inCurrentMonth && (
