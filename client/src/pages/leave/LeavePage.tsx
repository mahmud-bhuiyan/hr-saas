...
const TENANT_LEAVE_ROLES = ['company_admin', 'hr_manager', 'manager', 'employee'] as const;
...
export const LeavePage = () => {
  ...
  const [createForm, setCreateForm] = useState<CreateLeaveRequestInput>(emptyLeaveForm);
  const [actionLoadingId, setActionLoadingId] = useState<string | null>(null);
  const [calendarYear, setCalendarYear] = useState(now.getFullYear());
  const [calendarMonth, setCalendarMonth] = useState(now.getMonth() + 1);

  const canAccess =
    user && TENANT_LEAVE_ROLES.includes(user.role as (typeof TENANT_LEAVE_ROLES)[number]);
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <LeaveRequestsTable
              ...
              cancelLoadingId={cancelMutation.isPending ? actionLoadingId : null}
            />
          </div>
        </div>
      )}
