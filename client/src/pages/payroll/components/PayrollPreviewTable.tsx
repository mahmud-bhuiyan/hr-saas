...
export const PayrollPreviewTable = ({ summaries, loading }: PayrollPreviewTableProps) => {
  const totalGross = summaries.reduce((sum, row) => sum + row.grossEstimate, 0);
  const missingPayRateCount = summaries.filter((row) => row.missingPayRate).length;
    ...
    <div className="space-y-4">
      {missingPayRateCount > 0 && (
        <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-200">
          ...
          {missingPayRateCount === 1 ? 'has' : 'have'} no pay rate configured. Gross estimates
          show as zero until pay rates are set on employee profiles.
        </div>
      )}
        ...
        columns={[
          {
            ...
            render: (row) => (
              <div>
                <p className="font-medium text-slate-900 dark:text-slate-100">{row.employeeName}</p>
                {row.missingPayRate && (
                  <p className="text-xs text-amber-700 dark:text-amber-300">Missing pay rate</p>
                )}
              </div>
            ),
          },
          ...
          },
          {
            ...
            align: 'right',
            render: (row) => (
              <span className="font-medium">{formatMoney(row.grossEstimate, row.payCurrency)}</span>
            ),
          },
        ]}
      />
